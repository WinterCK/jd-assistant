...
class Assistant(object):
    ...
    def _validate_cookies(self):
        ...
        try:
            ...
            logger.info("校验cookies: 返回= {}%s", resp)
    ...
    def add_item_to_cart(self, sku_ids):
        ...
        for sku_id, count in parse_sku_id(sku_ids=sku_ids).items():
            ...
            if result:
                ...
                logger.error(soup)
                raise AsstException('添加购物车失败，请确认是否登陆已过期')
    ...
    def buy_item_sec_kill(self, times, sku_ids, area, submit_retry, submit_interval):
        items_dict = parse_sku_id(sku_ids)
        area_id = parse_area_id(area=area)
        while True:
            now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
            nowTimes = datetime.datetime.strptime(now, '%Y-%m-%d %H:%M:%S.%f')
            secKillTimes = datetime.datetime.strptime(times, '%Y-%m-%d %H:%M:%S.%f')
            logger.info("想要下单的秒杀时间为: %s, 当前时间: %s", secKillTimes, now)
            if nowTimes > secKillTimes:
                while True:
                    try:
                        for (sku_id, count) in items_dict.items():
